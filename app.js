...
const tweetAccessVerification = async (request, response, next) => {
  ...
  const getTweetQuery = `
    ...
    *
    FROM
    tweet INNER JOIN follower ON tweet.user_id = follower.following.user_id
    WHERE
    tweet.tweet_id = '${tweetId}' AND follwer_user_id='${userId};`
...
app.get(
  ...
  async (request, response) => {
    ...
    const {tweetId} = request.params
    const getTweetQuery = `
    SELECT tweet, (SELECT COUNT() FROM like tweet_id= '${tweetId}') AS likes,
...
app.get(
  ...
  async (request, response) => {
    const {tweetId} = request.params
    const getLikesQuery = `
    SELECT username FROM user INNER JOIN LIKE user.user_id = like.user_id WHERE tweet_id = '${tweetId}';
    `
...
app.post('/user/tweets/', authentication, async (request, response) => {
  ...
  await databse.run(createTweetQuery)
